export const WEBSITE_CREATION_STEPS = [
  {
    title: "Choose Your Perfect Template",
    points: [
      "Browse a wide range of industry-ready, professional templates.",
      "Preview in real-time to check responsiveness on all devices.",
      "Mix and match template sections for a unique design.",
      "Start with sleek, modern TailwindCSS styling out of the box."
    ],
    img: "/assets/website-creation-steps/template-selection.png",
  },
  {
    title: "Customize & Edit with Ease",
    points: [
      "Use AI Assistant to instantly rewrite text, restyle elements, or redesign sections.",
      "Easily update text, fonts, colors, layouts, and images with simple clicks.",
      "Rearrange or remove elements effortlessly — no worries about mistakes.",
      "For developers, tweak live code in HTML, React, or Next.js directly."
    ],
    img: "/assets/website-creation-steps/template-selection.png",
  },
  {
    title: "Preview & Export → Launch Website",
    points: [
      "Preview your website live across devices for pixel-perfect design.",
      "Export clean, production-ready code in HTML, React, or Next.js.",
      "Deploy seamlessly to Vercel, Netlify, or GitHub Pages.",
      "Follow simple deployment guides to launch your site instantly."
    ],
    img: "/assets/website-creation-steps/template-selection.png",
  },
];
