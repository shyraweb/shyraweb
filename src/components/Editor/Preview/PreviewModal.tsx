...
const ScreenSizeToggle = dynamic(
  () => import("@/components/Editor/Preview/ScreenSizeWrapper")
);
    ...
    <Drawer open={open} onOpenChange={onClose}>
      ...
      <DrawerContent className="w-full min-h-full">
        <DrawerHeader className="p-0 -mt-3 mb-3">
          <VisuallyHidden>
            <DrawerTitle>Preview Code</DrawerTitle>
          </VisuallyHidden>
          <div className="flex justify-between items-center px-4 ">
            <div className="flex gap-2 border rounded-md w-fit border-[#8B5CF6]">
              {PREVIEW_CODE_TABS.map((tab) => (
                <button
                  key={tab.id}
                  className={`flex gap-2 text-sm p-3 items-center text-center font-semibold ${
                    activeTab === tab.value
